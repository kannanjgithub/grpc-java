...
final class CelCommon {
  ...
  /**
   * Set of internal CEL overload IDs that are stable and do not have type-specific suffixes.
   */
  ...
  /**
   * Regular expression pattern to validate internal CEL overload IDs that may have
   * type-specific suffixes.
   *
   * <p>Standard CEL operators and conversion functions often have empty names in the AST
   * and are identified solely by their overload IDs (e.g., {@code divide_int64} for {@code /}).
   *
   * <p>This pattern matches allowed overload IDs by their prefixes (e.g., {@code divide},
   * {@code size}), optionally followed by numeric types (e.g., {@code int64}) and type-specific
  ...
  static void checkAllowedReferences(CelAbstractSyntaxTree ast) {
    for (Map.Entry<Long, CelReference> entry : ast.getReferenceMap().entrySet()) {
      ...
      if (!ref.value().isPresent() && ref.overloadIds().isEmpty()) {
        ...
        // Standard operators and functions have empty names and non-empty overload IDs.
        // If a name is present, it's a custom or unsupported named function.
        if (!ref.name().isEmpty()) {
          throw new IllegalArgumentException(
              "CEL expression references unsupported named function: " + ref.name());
        }
